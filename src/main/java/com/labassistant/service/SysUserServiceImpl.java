...
public class SysUserServiceImpl extends BaseAbstractService<SysUserEntity>
	...
	private String findPwdHtmlTemplate(String validUrl){
		...
		sb.append("</head>");    
		sb.append("<body>");  
		sb.append("<p>取回密码说明</p>");
		sb.append("<p>您收到这封邮件，是由于这个邮箱地址在 " + AppConfig.COMPANY + " 被登记为用户邮箱， 且该用户请求使用 Email 密码重置功能所致。</p>");
		sb.append("<p>------------------------------------------------------------------------</p>");
		...
		sb.append("<p>------------------------------------------------------------------------</p>");
		sb.append("<p>您只需在提交请求后的三天内，通过点击下面的链接重置您的密码：</p>");
		sb.append("<p>" + validUrl + "</p>");  
		sb.append("<p>(如果上面不是链接形式，请将该地址手工粘贴到浏览器地址栏再访问)。 ");
		sb.append("在上面的链接所打开的页面中输入新的密码后提交，您即可使用新的密码登录网站了。您可以在用户控制面板中随时修改您的密码。</p>"); 
		sb.append("<p>此致:</p>");
		sb.append("" + AppConfig.COMPANY + " 管理团队 ");
		sb.append("www.hualang-it.com.cn");
		sb.append("</body>");  
		sb.append("</html>");  	
		return sb.toString();
	}
